...
extension Game {
    ...
    /// An object whose sole function is to determine what piece nodes need to
    /// be moved based on a jump in the game's currentIndex.
    struct TraversalHandler {

        private let dataSource: PieceNodeDataSource
        //        private let game: Game

        init(dataSource: PieceNodeDataSource) {
            self.dataSource = dataSource
            //            self.game = game
        }

        func userDidTraverse(_ direction: Direction, with items: [HistoryItem]) {
        ...
        private func mobilize(_ items: [HistoryItem], toward direction: Direction) {
            ...
            func consolidate(items: [HistoryItem], direction: Direction) -> TransactionTable {
                ...
                func findNode(for square: Square) -> Piece.Node {
                    ...
                    if let candidate = dataSource.pieceNode(for: square) {
                ...
                func apprehend(item: HistoryItem) {
                    ...
                    if let capture = item.capture {
                        ...
                        case .undo:
                            let capturedNode = dataSource.pieceNode(for: capture.piece)
                    ...
                    if let promotion = item.promotion {
                        ...
                        result[dataSource.pieceNode(for: piece)] = Transaction(origin: move.origin, target: move.target, status: .added)
                    }
            ...
            func perform(_ transaction: Transaction, on pieceNode: Piece.Node) {
                if transaction.status == .removed {
                    dataSource.remove(pieceNode)
                } else if transaction.status == .captured {
                    dataSource.capture(pieceNode)
                } else {
                    if transaction.status == .added {
                        dataSource.add(pieceNode, at: transaction.origin)
                    } else if transaction.status == .resurrected {
                        dataSource.resurrect(pieceNode, at: transaction.origin)
                    }
                    dataSource.move(pieceNode, to: transaction.target)
                }
            }
